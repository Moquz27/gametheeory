...
\hypersetup{
    ...
    pdfauthor={Group 15b},
    pdfsubject={Linear Algebra Project Report},
    pdfkeywords={game theory, matrix games, linear programming}
}
...
The study of strategic decision-making has become very important in modern mathematics, and in many real-world situations, individuals, companies, and organizations must make decisions while considering the possible actions of their opponents. 

Matrix game theory, specifically two-player zero-sum games, provides a way to analyze competitive situations and determine optimal strategies. In practice, matrix games have applications in economics, business competition, and other fields where strategic interactions are significant factors. 
...
First, we focus on analyzing the main concepts such as pure strategy, mixed strategy, saddle point, and the Minimax Theorem. In addition, we aim to explain the mathematical relationship between matrix games and linear programming through the construction of linear programming (LP) models. 

Second, the group will explain the algorithms used to solve matrix games for both the \(2 \times 2\) case and the general case, using MATLAB to determine the optimal strategies and the value of the game. 
...
From an algorithmic perspective, the project focuses on studying methods for solving matrix games through linear programming and applying optimization algorithms to determine optimal solutions. To ensure a balance between theory and practical application, these methods will be applied to analyze three real-world applications in the socioeconomic field.
...
Matrix games are mathematical models used to analyze competitive situations between players with conflicting interests. In this model, the outcome is represented by a payoff matrix, where each element \(a_{ij}\) shows the payoff obtained when Player A chooses row strategy \(i\) and Player B chooses column strategy \(j\).

In this project, we focus on finite two-player zero-sum games. In this type of game, the total payoff of the two players is always zero, meaning that the gain of one player is exactly the loss of the other. Therefore, Player A aims to maximize the payoff, while Player B aims to minimize it \cite{osborne2004}.

The rows of the matrix usually represent the strategies of Player A, while the columns represent the strategies of Player B. A positive value of \(a_{ij}\) indicates an advantage for Player A, whereas a negative value indicates a disadvantage. Matrix games are widely applied in economics, military strategy, and social situations where decision makers must choose optimal strategies under competition and uncertainty.
...
\]
Here, \(x_i\) denotes the probability that Player A chooses strategy \(i\). Therefore, the probabilities must satisfy
\[
...
To determine a saddle point, two values are commonly used: \(\max_i \min_j a_{ij}\) and \(\min_j \max_i a_{ij}\). These values are called the maximin value and the minimax value, respectively.

The maximin principle is the strategy principle of Player A, who aims to maximize the minimum payoff that can be guaranteed. It is found by taking the minimum value of each row and then choosing the largest value among them.

The minimax principle is the strategy principle of Player B, who aims to minimize the maximum possible payoff of Player A. It is found by taking the maximum value of each column and then choosing the smallest value among them.
...
Consider each row: 
\[
\min(A1)=1,\ \min(A2)=2, \ \max(1,2)=2
\]
\[
\Rightarrow \text{maximin}=2
\]
Consider each column: 
\[
\max(B1)=5, \ \max(B2)=2,\ \min(5,2)=2
\]
\[
\Rightarrow \text{minimax}=2
\]
...
\]
Here, \(v\) is the value of the game \cite{thie2008}.
...
\[
x_i \ge 0 \quad \text{and} \quad y_j \ge 0
\]
Their total probabilities also satisfy:
\[
\sum_i x_i = 1 \quad \text{and} \quad \sum_j y_j = 1
\]
...
A=(a_{ij})
\]
Here, \(a_{ij}\) represents the payoff received by Player A when:

\begin{itemize}
    \item Player A chooses row strategy \(i\).
    \item Player B chooses column strategy \(j\).
\end{itemize}
...
a'_{ij}=a_{ij}+c
\]
The purpose of this transformation is to ensure that all elements of the new matrix are positive, improve numerical stability during optimization, and avoid division by zero or negative values.

After the transformation, the new matrix becomes:
\[
A'=(a'_{ij})
\]
Moreover, adding the same constant to all payoff values does not change the optimal strategies of the players because every payoff increases equally. Only the value of the game changes:
\[
v'=v+c
\]
Here:
...
\begin{itemize}
    \item \(v\): original game value
    \item \(c\): added positive constant
    \item \(v'\): shifted game value
\end{itemize}
...
v=v'-c
\]
will recover the original game value \cite{murthy2007}.
...
\begin{itemize}
    \item \(x_i\): probability of choosing row strategy \(i\)
    \item \(x_i \ge 0\)
\end{itemize}
...
\vspace{0.1cm}
\[
\sum_{i=1}^{m}a'_{ij}x_i \ge v'
\quad \forall j=1,\dots,n
\]
Here:

\begin{itemize}
    \item \(a'_{ij}\): payoff at row \(i\), column \(j\)
    \item \(x_i\): probability that Player A chooses row \(i\)
    \item \(\sum_{i=1}^{m}a'_{ij}x_i\): expected payoff of Player A if Player B selects column \(j\).
\end{itemize}
...
Subject to:
\[
\sum_{i=1}^{m}a'_{ij}X_i \ge 1
\]
\vspace{0.1cm}
The transformation from ``maximize \(v'\)'' to ``minimize \(\sum_{i=1}^{m} X_i\)'' is derived as follows.
...
Therefore:
\[
\sum_{i=1}^{m}X_i=\frac1{v'}
\]
\vspace{0.1cm}
This means that when \(v'\) increases, \(\frac1{v'}\) decreases. Hence:
\[
\max(v') \iff \min\left(\frac1{v'}\right)
\]
The equality below gives the equivalent objective:
\[
\frac1{v'}=\sum_{i=1}^{m}X_i
\]
Therefore, the problem becomes equivalent to minimizing:
\[
\sum_{i=1}^{m}X_i
\]
...
This leads to:
\[
\sum_{j=1}^{n}a'_{ij}y_j \le v'
\]
Here:
\[
\sum_{j=1}^{n}a'_{ij}y_j
\]
This represents the expected payoff of Player A when Player A selects row \(i\).
...
Subject to:
\[
\sum_{j=1}^{n}a'_{ij}Y_j \le 1
\] \cite{thie2008}
With:
\[
Y_j \ge 0
\]
The transformation follows the same logic as Player A's formulation. The normalization condition is:
\vspace{0.1cm}
\[
\sum_{j=1}^{n}y_j=1
\]
...
Therefore:
\[
\sum_{j=1}^{n}Y_j=\frac1{v'}
\]
\vspace{0.1cm}
Therefore, minimizing the upper bound \(v'\) is equivalent to maximizing:
\vspace{0.1cm}
\[
\sum_{j=1}^{n}Y_j
\]
...
\subsubsection{Algorithm for Solving \(2 \times 2\) Matrix Games}
...
\] \cite{thie2008}
\subsubsection{Algorithm for General \(m \times n\) Matrix Games}

The direct formula used for the \(2 \times 2\) case is not suitable for matrix games with larger dimensions. Therefore, linear programming is required. In the general case, Player A may have \(m\) strategies and Player B may have \(n\) strategies. The payoff matrix can be written as:
\[
A = (a_{ij})_{m \times n}
\]
The general algorithm still follows the same initial structure as the \(2 \times 2\) case. First, the minimum value of each row, the maximum value of each column, the maximin value, and the minimax value are computed. Then, the algorithm checks whether the game has a saddle point.
...
\[
x=[0,\ 0,\ 0,\ 0.4167,\ 0.3333,\ 0.25]
\]
This means that Country A should not focus on a single area. Instead, it should prioritize combined strategies: Alpha-Bravo with probability $41.67\%$, Alpha-Charlie with probability $33.33\%$, and Bravo-Charlie with probability $25\%$. Alpha-Bravo has the highest probability because high technology and supply chains are the two most strategically valuable areas.

The optimal mixed strategy of Country B is
\[
y=[0,\ 0,\ 0,\ 0.25,\ 0.3333,\ 0.4167]
\]
...
In this scenario, we consider a social conflict between Urban Management Authority A and informal street vendors (Group B) in a densely populated urban area. The authority wants to maintain public order, reduce congestion, and preserve urban hygiene, while the street vendors want to continue selling on the sidewalk in order to maximize their income. The game cycle is counted weekly, and each side cannot change its strategy until the end of the week.

Urban Management Authority A has three strategies. A1 is giving warnings and conducting one regular inspection per day. A2 is issuing fines and conducting continuous patrols by shifts throughout the day. A3 is coordinating multiple departments, conducting one large-scale weekly sweep, and imposing heavy fines. The informal vendors in Group B have four strategies. B1 is temporary compliance, B2 is selling during low-inspection hours, B3 is leaving during inspection and returning afterward, and B4 is coordinating as a group to avoid inspection and frequently changing locations.
